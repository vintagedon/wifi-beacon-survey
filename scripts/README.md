...
|------|-------------|--------|
| [wifi-beacon-sample.sh](wifi-beacon-sample.sh) | Derives permitted frequencies from live kernel regulatory state, tunes each in turn with `iw`, captures beacons with tcpdump, and writes per-frequency PCAP plus a sweep manifest | ⚠️ Discovery prototype; production completion contract pending |
...
The analysis tools are read-only and take a sweep directory:

```bash
python3 analyze-sweep.py /opt/agents/repos/storage-mounted/wifi-beacon-survey/sweeps/<sweep-id>
python3 probe-surfaces.py /opt/agents/repos/storage-mounted/wifi-beacon-survey/sweeps/<sweep-id> --tsv ./out
